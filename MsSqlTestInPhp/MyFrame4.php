...
class MyFrame4 extends wxFrame
{
    ...
    function GetData()
    {
        try
        {
            ...
            $relations = array();
            $cursor = odbc_exec($connection, "SELECT * FROM [MsSqlTestDatabase].[dbo].[Relations] WHERE IsDeleted = 0");

            // Store the items in a Person list.
            while(odbc_fetch_row($cursor))
            {
                $relations[] = odbc_result($cursor, 1);
                $relations[] = odbc_result($cursor, 2);
                $relations[] = odbc_result($cursor, 3);
                $relations[] = odbc_result($cursor, 4);
                $relations[] = odbc_result($cursor, 5);
            }
    ...
    function m_grid1OnGridSelectCell( $event )
    {
        ...
        if ($event->GetRow() >= 0)
        {
            try
            {
                ...
                // Get the items from the Relations table.
                $relation = array();
                $cursor = odbc_exec($connection, "SELECT * FROM [MsSqlTestDatabase].[dbo].[Relations] WHERE Id = '$this->idValue'");

                // Store it in a Relation object.
                while(odbc_fetch_row($cursor))
                {
                    $relation[] = odbc_result($cursor, 1);
                    $relation[] = odbc_result($cursor, 2);
                    $relation[] = odbc_result($cursor, 3);
                    $relation[] = odbc_result($cursor, 4);
                    $relation[] = odbc_result($cursor, 5);
                }
    ...
    function m_comboBox1OnCombobox( $event )
    {
        try
        {
            ...
            // Get the items from the People table.
            $retVal = array();
            $cursor = odbc_exec($connection, "SELECT * FROM [MsSqlTestDatabase].[dbo].[People] WHERE Id = '$this->name1Value'");

            // Store it in a Person object.
            while(odbc_fetch_row($cursor))
            {
                $retVal[] = odbc_result($cursor, 1);
                $retVal[] = odbc_result($cursor, 2);
                $retVal[] = odbc_result($cursor, 3);
                $retVal[] = odbc_result($cursor, 4);
                $retVal[] = odbc_result($cursor, 5);
                $retVal[] = odbc_result($cursor, 6);
            }
    ...
    function m_comboBox2OnCombobox( $event )
    {
        try
        {
            ...
            // Get the items from the Relationships table.
            $retVal = array();
            $cursor = odbc_exec($connection, "SELECT * FROM [MsSqlTestDatabase].[dbo].[Relationships] WHERE Id = '$this->relationshipValue'");
    ...
    function m_comboBox3OnCombobox( $event )
    {
        try
        {
            ...
            // Get the items from the People table.
            $retVal = array();
            $cursor = odbc_exec($connection, "SELECT * FROM [MsSqlTestDatabase].[dbo].[People] WHERE Id = '$this->name2Value'");
    ...
    function m_button2OnButtonClick( $event )
    {
        ...
        try
        {
                ...
                $this->name1Value > 0 && $this->relationshipValue > 0 && $this->name2Value > 0 && $this->name1Value != $this->name2Value)
            {
                ...
                // Get the items from the Relations table.
                $cursor = odbc_exec($connection, "SELECT * FROM [MsSqlTestDatabase].[dbo].[Relations]");
                ...
                while(odbc_fetch_row($cursor))
                {
                    ...
                    if ($matchFound == false && odbc_result($cursor, 2) == $this->name1Value && odbc_result($cursor, 3) == $this->relationshipValue && 
                        odbc_result($cursor, 4) == $this->name2Value && odbc_result($cursor, 5) == true)
                    {
                        $matchFound = true;
                ...
                if ($matchFound == false)
                {
                    ...
                    $cursor = odbc_exec($connection, "INSERT INTO [MsSqlTestDatabase].[dbo].[Relations] (Id, Person1, Relationship, Person2, IsDeleted) VALUES ('$this->idValue' + 1, '$this->name1Value', '$this->relationshipValue', '$this->name2Value', 0)");
                }
                else
                {
                    // Edit the IsDeleted value to false.
                    $cursor = odbc_exec($connection, "UPDATE [MsSqlTestDatabase].[dbo].[Relations] SET IsDeleted = 0 WHERE Id = '$this->idValue'");
                }
        ...
        try
        {
                ...
                $this->name1Value > 0 && $this->relationshipValue > 0 && $this->name2Value > 0 && $this->name1Value != $this->name2Value)
            {
                ...
                if ($this->selectedIndexValue >= 0)
                {
                    ...
                    $cursor = odbc_exec($connection, "UPDATE [MsSqlTestDatabase].[dbo].[Relations] SET Person1 = '$this->name1Value', Relationship = '$this->relationshipValue', Person2 = '$this->name2Value' WHERE Id = '$this->idValue'");
    ...
    function m_button4OnButtonClick( $event )
    {
        ...
        try
        {
            ...
            if ($this->selectedIndexValue >= 0)
            {
                ...
                $cursor = odbc_exec($connection, "UPDATE [MsSqlTestDatabase].[dbo].[Relations] SET IsDeleted = 1 WHERE Id = '$this->idValue'");
