...
class MyFrame3 ( wx.Frame ):
	...
	def GetData(self):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM [MsSqlTestDatabase].[dbo].[Relationships] WHERE IsDeleted = 0')
			...
			if (self.__nameValue <> None and self.__nameValue <> ''):
				...
				if (__matchFound == False):
					...
					if (__deletedFound == False):
						...
						__cursor.execute('INSERT INTO [MsSqlTestDatabase].[dbo].[Relationships] (Id, Name, IsDeleted) VALUES (%s, %s, %s)',
						...
						__cursor.execute('UPDATE [MsSqlTestDatabase].[dbo].[Relationships] SET IsDeleted = %s WHERE Id = %s',
		...
		try:
			...
			if (self.__nameValue <> None and self.__nameValue <> ''):
				...
				if (self.__selectedIndexValue >= 0):
					...
					# Execute.
					__cursor.execute('UPDATE [MsSqlTestDatabase].[dbo].[Relationships] SET Name = %s WHERE Id = %s',
