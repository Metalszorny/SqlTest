...
class MyFrame2 extends wxFrame
{
    ...
    function GetData()
    {
        try
        {
            ...
            $people = array();
            $cursor = mysqli_query($connection, "SELECT * FROM MySqlTestDatabase.People WHERE IsDeleted = 0");
            ...
            for ($i = 0; $i < count($people) / 6; $i++)
            {
                ...
                $this->m_grid1->SetCellValue($this->m_grid1->GetNumberRows() - 1, 4, $people[$i * 6 + 4]);
            }
        }
        catch (Exception $ex)
        { }
        ...
        try
        {
                ...
                $this->nameValue != "" && $this->mothernameValue != "" && $this->locationValue != "" && $this->birthdateValue != "")
            {
                ...
                foreach ($result as $oneRow)
                {
                    ...
                    if ($matchFound == false && $oneRow[1] == $this->nameValue && $oneRow[2] == $this->mothernameValue && 
                        $oneRow[3] == $this->locationValue && $oneRow[4] == $this->birthdateValue && $oneRow[5] == true)
                    {
                        $matchFound = true;
                ...
                if ($matchFound == false)
                {
                    ...
                    $cursor = mysqli_query($connection, "INSERT INTO MySqlTestDatabase.People (Id, Name, Mothername, Location, Birthdate, IsDeleted) VALUES ('$this->idValue' + 1, '$this->nameValue', '$this->mothernameValue', '$this->locationValue', '$this->birthdateValue' , false)");
                }
                else
                {
                    // Edit the IsDeleted value to false.
                    $cursor = mysqli_query($connection, "UPDATE MySqlTestDatabase.People SET IsDeleted = false WHERE Id = '$this->idValue'");
                }
        ...
        try
        {
                ...
                $this->nameValue != "" && $this->mothernameValue != "" && $this->locationValue != "" && $this->birthdateValue != "")
            {
                ...
                if ($this->selectedIndexValue >= 0)
                {
                    ...
                    $cursor = mysqli_query($connection, "UPDATE MySqlTestDatabase.People SET Name = '$this->nameValue', Mothername = '$this->mothernameValue', Location = '$this->locationValue', Birthdate = '$this->birthdateValue' WHERE Id = '$this->idValue'");
    ...
    function m_button4OnButtonClick( $event )
    {
        ...
        try
        {
            ...
            if ($this->selectedIndexValue >= 0)
            {
                ...
                $cursor = mysqli_query($connection, "UPDATE MySqlTestDatabase.People SET IsDeleted = true WHERE Id = '$this->idValue'");
