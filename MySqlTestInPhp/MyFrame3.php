...
class MyFrame3 extends wxFrame
{
    ...
    function GetData()
    {
        try
        {
            ...
            $relationships = array();
            $cursor = mysqli_query($connection, "SELECT * FROM MySqlTestDatabase.Relationships WHERE IsDeleted = 0");
        ...
        try
        {
            ...
            if ($this->nameValue != null && $this->nameValue != "")
            {
                ...
                foreach ($result as $oneRow)
                {
                    ...
                    if ($matchFound == false && $oneRow[1] == $this->nameValue && $oneRow[2] == false)
                    {
                        $matchFound = true;
                    ...
                    if ($deletedFound == false && $oneRow[1] == $this->nameValue && $oneRow[2] == true)
                    {
                        $deletedFound = true;
                ...
                if ($matchFound == false)
                {
                    ...
                    if ($deletedFound == false)
                    {
                        ...
                        $cursor = mysqli_query($connection, "INSERT INTO MySqlTestDatabase.Relationships (Id, Name, IsDeleted) VALUES ('$this->idValue' + 1, '$this->nameValue', false)");
                    }
                    else
                    {
                        // Edit the IsDeleted value to false.
                        $cursor = mysqli_query($connection, "UPDATE MySqlTestDatabase.Relationships SET IsDeleted = false WHERE Id = '$this->idValue'");
                    }
                }
        ...
        try
        {
            ...
            if ($this->nameValue != null && $this->nameValue != "")
            {
                ...
                if ($this->selectedIndexValue >= 0)
                {
                    ...
                    // Update the values in the Relationships table.
                    $cursor = mysqli_query($connection, "UPDATE MySqlTestDatabase.Relationships SET Name = '$this->nameValue' WHERE Id = '$this->idValue'");
    ...
    function m_button4OnButtonClick( $event )
    {
        ...
        try
        {
            ...
            if ($this->selectedIndexValue >= 0)
            {
                ...
                $cursor = mysqli_query($connection, "UPDATE MySqlTestDatabase.Relationships SET IsDeleted = true WHERE Id = '$this->idValue'");
