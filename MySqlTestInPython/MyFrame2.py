...
class MyFrame2 ( wx.Frame ):
    ...
    def GetData(self):
        try:
            ...
            # Execute.
            __cursor.execute('SELECT * FROM MySqlTestDatabase.People WHERE IsDeleted = 0')
            ...
            for __oneItem in __people:
                ...
                self.m_grid1.SetCellValue(self.m_grid1.GetNumberRows() - 1, 4, str(__oneItem[4]))
        except:
            pass
            ...
            if (self.__nameValue <> None and self.__mothernameValue <> None and self.__locationValue <> None and self.__birthdateValue <> None and
                ...
                for __row in __cursor.fetchall():
                    ...
                    if (__matchFound == False and str(__row[1]) == self.__nameValue and str(__row[2]) == self.__mothernameValue and
                        str(__row[3]) == self.__locationValue and str(__row[4]) == self.__birthdateValue and __row[5] == True):
                        __matchFound = True
                ...
                if (__matchFound == False):
                    ...
                    __cursor.execute('INSERT INTO MySqlTestDatabase.People (Id, Name, Mothername, Location, Birthdate, IsDeleted) VALUES (%s, %s, %s, %s, %s, %s)',
                                     (self.__idValue + 1, self.__nameValue, self.__mothernameValue, self.__locationValue, self.__birthdateValue, False))

                    # Execute.
                    __connection.commit()
                else:
                    # Edit the IsDeleted value to false.
                    __cursor.execute('UPDATE MySqlTestDatabase.People SET IsDeleted = %s WHERE Id = %s',
        ...
        try:
            ...
            if (self.__nameValue <> None and self.__mothernameValue <> None and self.__locationValue <> None and self.__birthdateValue <> None and
                ...
                if (self.__selectedIndexValue >= 0):
                    ...
                    # Execute.
                    __cursor.execute('UPDATE MySqlTestDatabase.People SET Name = %s, Mothername = %s, Location = %s, Birthdate = %s WHERE Id = %s',
                                     (self.__nameValue, self.__mothernameValue, self.__locationValue, self.__birthdateValue, self.__idValue))
