...
class MyFrame4 ( wx.Frame ):
	...
	def GetData(self):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM MySqlTestDatabase.People')
			...
			__relations = []
			__cursor.execute('SELECT * FROM MySqlTestDatabase.Relations WHERE IsDeleted = 0')
	...
	def m_grid1OnGridSelectCell( self, event ):
		...
		if (event.GetRow() >= 0):
			try:
				...
				# Execute.
				__cursor.execute('SELECT * FROM MySqlTestDatabase.Relations WHERE Id = %d' % (self.__idValue))
	...
	def m_comboBox1OnCombobox( self, event ):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM MySqlTestDatabase.People WHERE Id = %d' % (self.__name1Value))
	...
	def m_comboBox2OnCombobox( self, event ):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM MySqlTestDatabase.Relationships WHERE Id = %d' % (self.__relationshipValue))
	...
	def m_comboBox3OnCombobox( self, event ):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM MySqlTestDatabase.People WHERE Id = %d' % (self.__name2Value))
	...
	def m_button2OnButtonClick( self, event ):
		...
		try:
			...
			if (self.__name1Value <> None and self.__relationshipValue <> None and self.__name2Value <> None and
				...
				# Execute.
				__cursor.execute('SELECT * FROM MySqlTestDatabase.Relations')
				...
				if (__matchFound == False):
					...
					__cursor.execute('INSERT INTO MySqlTestDatabase.Relations (Id, Person1, Relationship, Person2, IsDeleted) VALUES (%s, %s, %s, %s, %s)',
					...
					__cursor.execute('UPDATE MySqlTestDatabase.Relations SET IsDeleted = %s WHERE Id = %s',
		...
		try:
			...
			if (self.__name1Value <> None and self.__relationshipValue <> None and self.__name2Value <> None and
				...
				if (self.__selectedIndexValue >= 0):
					...
					# Execute.
					__cursor.execute('UPDATE MySqlTestDatabase.Relations SET Person1 = %s, Relationship = %s, Person2 = %s WHERE Id = %s',
