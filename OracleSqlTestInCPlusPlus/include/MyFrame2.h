...
#include <vector>
#include <Person.h>
#include <C:\Oraclex86\ocilib\include\ocilib.h>
	...
	private:
		...
		virtual void m_button2OnButtonClick( wxCommandEvent& event )
		{
		    ...
		    try
		    {
                    ...
                    this->name != "" && this->mothername != "" && this->location != "" && this->birthdate != "")
                {
                    ...
                    // Connect to the DSN.
                    SQLDriverConnect(sqlconnectionhandle, NULL, (SQLWCHAR*)L"DSN=orcl;SERVER=localhost;UID=root;PWD=root;DRIVER=Oracle Server;", SQL_NTS, retconstring, 1024, NULL, SQL_DRIVER_NOPROMPT);

                    // Get the items from the People table.
                    // Allocate statement handle.
                    SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle, &sqlstatementhandle);
                    // Process data.
                    SQLExecDirect(sqlstatementhandle, (SQLWCHAR*)L"SELECT * FROM roote.People", SQL_NTS);
                    ...
                    while(SQLFetch(sqlstatementhandle) == SQL_SUCCESS)
                    {
                        ...
                        SQLGetData(sqlstatementhandle, 6, SQL_C_ULONG, &isDeletedValue, 0, NULL);
                    ...
                    if (matchFound == false)
                    {
                        ...
                        SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle, &sqlstatementhandle);
                        // Process data.
                        SQLPrepare(sqlstatementhandle, (SQLWCHAR*)L"INSERT INTO root.People (Id, Name, Mothername, Location, Birthdate, IsDeleted) VALUES (?, ?, ?, ?, ?, 0)", SQL_NTS);
                    ...
                    else
                    {
                        ...
                        SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle, &sqlstatementhandle);
                        // Process data.
                        SQLPrepare(sqlstatementhandle, (SQLWCHAR*)L"UPDATE root.People SET IsDeleted = 0 WHERE Id = ?", SQL_NTS);
		    ...
		    try
		    {
                    ...
                    this->name != "" && this->mothername != "" && this->location != "" && this->birthdate != "")
                {
                    ...
                    /*if (this->selectedIndex >= 0)
                    {
                        ...
                        SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle, &sqlstatementhandle);
                        // Process data.
                        SQLPrepare(sqlstatementhandle, (SQLWCHAR*)L"UPDATE root.People SET Name = ?, Mothername = ?, Location = ?, Birthdate = ? WHERE Id = ?", SQL_NTS);
		...
		virtual void m_button4OnButtonClick( wxCommandEvent& event )
		{
		    ...
		    try
		    {
                ...
                if (this->selectedIndex >= 0)
                {
                    ...
                    SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle, &sqlstatementhandle);
                    // Process data.
                    SQLPrepare(sqlstatementhandle, (SQLWCHAR*)L"UPDATE root.People SET IsDeleted = 1 WHERE Id = ?", SQL_NTS);
		...
		virtual void GetData()
		{
		    try
		    {
                ...
                cn = OCI_ConnectionCreate("localhost/orcl", "root", "root", OCI_SESSION_DEFAULT);
                OCI_Statement* st = NULL;
                st = OCI_StatementCreate(cn);
                OCI_Prepare(st, "SELECT * FROM root.People WHERE IsDeleted = 0");
                //OCI_Execute(st);
                //OCI_Commit(cn);
                ...
                {
                    int idValue = OCI_GetInt(rs, 1);
                    string nameValue = OCI_GetString(rs, 2);
                    string mothernameValue = OCI_GetString(rs, 3);
                    string locationValue = OCI_GetString(rs, 4);
                    OCI_Timestamp* temp1 = OCI_GetTimestamp(rs, 5);
                    char dateValue[20];
                    OCI_TimestampToText(temp1, "YYYY.MM.DD", 19, dateValue, 3);
                    /*TIMESTAMP_STRUCT birthdateValue = temp1;
                    int isDeletedIntValue = OCI_GetInt(rs, 6);

                    bool isDeletedValue;
                    if (isDeletedIntValue > 0)
                    {
                        isDeletedValue = true;
                    }
                    else
                    ...
                    Person* person = new Person(idValue, nameValue, mothernameValue, locationValue, birthdateValue, isDeletedValue);

                    //people->push_back(person);
                    people->insert(people->end(), person);*/
                    this->m_grid1->AppendRows(1);
                    this->m_grid1->SetCellValue(this->m_grid1->GetNumberRows() - 1, 0, wxString::Format(wxT("%i"), idValue));
                    this->m_grid1->SetCellValue(this->m_grid1->GetNumberRows() - 1, 1, nameValue);
                    this->m_grid1->SetCellValue(this->m_grid1->GetNumberRows() - 1, 2, mothernameValue);
                    this->m_grid1->SetCellValue(this->m_grid1->GetNumberRows() - 1, 3, locationValue);
                    this->m_grid1->SetCellValue(this->m_grid1->GetNumberRows() - 1, 4, string(dateValue));
               }
               OCI_Cleanup();
                ...
                SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle, &sqlstatementhandle);
                // Process data.
                SQLExecDirect(sqlstatementhandle, (SQLWCHAR*)L"SELECT * FROM root.People WHERE IsDeleted = 0", SQL_NTS);
                ...
                while(SQLFetch(sqlstatementhandle) == SQL_SUCCESS)
                {
                    ...
                    SQLGetData(sqlstatementhandle, 4, SQL_C_CHAR, locationValue, 50, NULL);
                    SQLGetData(sqlstatementhandle, 5, SQL_C_DATE, &birthdateValue, 0, NULL);
                    SQLGetData(sqlstatementhandle, 6, SQL_C_ULONG, &isDeletedIntValue, 0, NULL);
