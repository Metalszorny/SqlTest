...
class MyFrame3 extends wxFrame
{
    ...
    function GetData()
    {
        try
        {
            ...
            $relationships = array();
            $command = oci_parse($connection, "SELECT * FROM root.Relationships WHERE IsDeleted = 0");
            ...
            for ($i = 0; $i < count($relationships) / 3; $i++)
            {
                ...
                $this->m_grid1->SetCellValue($this->m_grid1->GetNumberRows() - 1, 0, $relationships[$i + 0 * (count($relationships) / 3)]);
                $this->m_grid1->SetCellValue($this->m_grid1->GetNumberRows() - 1, 1, $relationships[$i + 1 * (count($relationships) / 3)]);
            }
        }
        catch (Exception $ex)
        { }
        ...
        try
        {
            ...
            if ($this->nameValue != null && $this->nameValue != "")
            {
                ...
                for ($i = 0; $i < count($relationships) / 3; $i++)
                {
                    ...
                    if ($matchFound == false && 
						$relationships[$i + 1 * (count($relationships) / 3)] == $this->nameValue && 
						$relationships[$i + 2 * (count($relationships) / 3)] == 0)
                    {
                        $matchFound = true;
                    ...
                    if ($deletedFound == false && 
						$relationships[$i + 1 * (count($relationships) / 3)] == $this->nameValue && 
						$relationships[$i + 2 * (count($relationships) / 3)] == 1)
                    {
                        $deletedFound = true;
                        $this->idValue = $relationships[$i + 0 * (count($relationships) / 3)];
                ...
                if ($matchFound == false)
                {
                    ...
                    if ($deletedFound == false)
                    {
                        ...
                        $command = oci_parse($connection, "INSERT INTO root.Relationships (Id, Name, IsDeleted) VALUES (:param1, :param2, :param3)");
                        ...
                        oci_bind_by_name($command, ':param3', $falseValue);
                    ...
                    else
                    {
                        ...
                        $command = oci_parse($connection, "UPDATE root.Relationships SET IsDeleted = :param1 WHERE Id = :param2");
        ...
        try
        {
            ...
            if ($this->nameValue != null && $this->nameValue != "")
            {
                ...
                if ($this->selectedIndexValue >= 0)
                {
                    ...
                    $command = oci_parse($connection, "UPDATE root.Relationships SET Name = :param1 WHERE Id = :param2");
