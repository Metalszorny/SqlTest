...
class MyFrame3 ( wx.Frame ):
	...
	def GetData(self):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM root.Relationships WHERE IsDeleted = 0')
		...
		try:
			...
			if (self.__nameValue <> None and self.__nameValue <> ''):
				...
				for __row in __cursor.fetchall():
					...
					if (__matchFound == False and str(__row[1]) == self.__nameValue and __row[2] == 0):
						__matchFound = True

						break

					# Check if a deleted already exists.
					if (__deletedFound == False and str(__row[1]) == self.__nameValue and __row[2] == 1):
						__deletedFound = True
				...
				if (__matchFound == False):
					...
					if (__deletedFound == False):
						...
						__cursor.execute('INSERT INTO root.Relationships (Id, Name, IsDeleted) VALUES (:param1, :param2, :param3)',
										param1 = self.__idValue + 1, param2 = self.__nameValue, param3 = 0)

						# Execute.
						__connection.commit()
					else:
						# Edit the IsDeleted value to false.
						__cursor.execute('UPDATE root.Relationships SET IsDeleted = :param1 WHERE Id = :param2',
		...
		try:
			...
			if (self.__nameValue <> None and self.__nameValue <> ''):
				...
				if (self.__selectedIndexValue >= 0):
					...
					# Execute.
					__cursor.execute('UPDATE root.Relationships SET Name = :param1 WHERE Id = :param2',
                                     param1 = self.__nameValue, param2 = self.__idValue)
