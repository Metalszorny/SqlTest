...
class MyFrame4 ( wx.Frame ):
	...
	def GetData(self):
		try:
			...
			__relationships = []
			__cursor.execute('SELECT * FROM root.Relationships')
			...
			__relations = []
			__cursor.execute('SELECT * FROM root.Relations WHERE IsDeleted = 0')
	...
	def m_grid1OnGridSelectCell( self, event ):
		...
		if (event.GetRow() >= 0):
			try:
				...
				# Execute.
				__cursor.execute('SELECT * FROM root.Relations WHERE Id = %d' % (self.__idValue))
	...
	def m_comboBox1OnCombobox( self, event ):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM root.People WHERE Id = %d' % (self.__name1Value))
	...
	def m_comboBox2OnCombobox( self, event ):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM root.Relationships WHERE Id = %d' % (self.__relationshipValue))
	...
	def m_comboBox3OnCombobox( self, event ):
		try:
			...
			# Execute.
			__cursor.execute('SELECT * FROM root.People WHERE Id = %d' % (self.__name2Value))
	...
	def m_button2OnButtonClick( self, event ):
		...
		try:
			...
			if (self.__name1Value <> None and self.__relationshipValue <> None and self.__name2Value <> None and
				...
				# Execute.
				__cursor.execute('SELECT * FROM root.Relations')
				...
				for __row in __cursor.fetchall():
					...
					if (__matchFound == False and int(__row[1]) == self.__name1Value and int(__row[2]) == self.__relationshipValue and
						int(__row[3]) == self.__name2Value and __row[4] == 1):
						__matchFound = True
				...
				if (__matchFound == False):
					...
					__cursor.execute('INSERT INTO root.Relations (Id, Person1, Relationship, Person2, IsDeleted) VALUES (:param1, :param2, :param3, :param4, :param5)',
									param1 = self.__idValue + 1, param2 = self.__name1Value, param3 = self.__relationshipValue,
									param4 = self.__name2Value, param5 = 0)

					# Execute.
					__connection.commit()
				else:
					# Edit the IsDeleted value to false.
					__cursor.execute('UPDATE root.Relations SET IsDeleted = :param1 WHERE Id = :param2',
		...
		try:
			...
			if (self.__name1Value <> None and self.__relationshipValue <> None and self.__name2Value <> None and
				...
				if (self.__selectedIndexValue >= 0):
					...
					# Execute.
					__cursor.execute('UPDATE root.Relations SET Person1 = :param1, Relationship = :param2, Person2 = :param3 WHERE Id = :param4',
									param1 = self.__name1Value, param2 = self.__relationshipValue, param3 = self.__name2Value,
									param4 = self.__idValue)
